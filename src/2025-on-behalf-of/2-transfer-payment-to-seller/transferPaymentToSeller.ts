/**
 * A script which transfers a payment from imin's Stripe account to the seller's
 * Stripe account.
 *
...
import { iminStripe } from '../../common/consts.js';
...
if (!process.env.SELLER_STRIPE_ACCOUNT_ID) {
  throw new Error(
    'Env var SELLER_STRIPE_ACCOUNT_ID is required (e.g. SELLER_STRIPE_ACCOUNT_ID=acct_...)',
  );
}

const PAYMENT_INTENT_ID = process.env.PAYMENT_INTENT_ID;
const SELLER_STRIPE_ACCOUNT_ID = process.env.SELLER_STRIPE_ACCOUNT_ID;

// Run
main();

async function main() {
  const paymentIntent = await iminStripe.paymentIntents.retrieve(
    PAYMENT_INTENT_ID,
    {
